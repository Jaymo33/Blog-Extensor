...
try:
    ...
    # Fallback: explicitly load ../.env relative to this file
    root_env = os.path.normpath(os.path.join(os.path.dirname(__file__), "..", ".env"))
    if not (os.getenv("DEEPSEEK_API_KEY") or os.getenv("DEESEEK_API_KEY")) and os.path.exists(root_env):
        load_dotenv(root_env, override=True)
    # Minimal debug of env detection (masked)
    debug_info = {
        "found_dotenv": bool(dotenv_found),
        "root_env_exists": os.path.exists(root_env),
        "has_key": bool(os.getenv("DEEPSEEK_API_KEY") or os.getenv("DEESEEK_API_KEY")),
    }
    print(f"[env][WT-2] {debug_info}")
except Exception:
    pass

DEESEEK_API_KEY = (os.getenv("DEEPSEEK_API_KEY") or os.getenv("DEESEEK_API_KEY") or "").strip()
if not DEESEEK_API_KEY:
    print("[✗] DEEPSEEK_API_KEY is not set. Please add it to your .env.")
...
def generate_blog_from_row(row_data):
    if not DEESEEK_API_KEY:
        raise RuntimeError("DEEPSEEK_API_KEY missing. Set it in .env or environment and retry.")
