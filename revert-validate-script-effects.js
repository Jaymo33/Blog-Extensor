...
console.log('🔄 Reverting effects of validate-and-fix-schema.js script...');
...
function revertValidateScriptEffects(filePath) {
  try {
    let content = fs.readFileSync(filePath, 'utf8');
    const originalContent = content;
    
    // Check if this file has the problematic patterns from the validate script
    if (content.includes('schema: |') && content.includes('<script type="application/ld+json">')) {
      const relativePath = path.relative(blogDir, filePath);
      
      let fixedContent = content;
      
      // Fix 1: Convert 2-space indentation to 4-space indentation in JSON-LD blocks
      // This is the main issue caused by JSON.stringify(schema, null, 2)
      
      // Pattern: Fix lines that start with 2 spaces followed by JSON content
      ...
      // Fix 3: Fix missing closing braces - look for patterns like "inLanguage": "en-GB" followed by } without proper closing
...
function main() {
  try {
    ...
    files.forEach((filePath, index) => {
      ...
      try {
        const wasFixed = revertValidateScriptEffects(filePath);
        if (wasFixed) {
          fixedCount++;
        }
      } catch (error) {
