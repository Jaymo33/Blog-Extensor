...
console.log('🔄 Applying template structure EXACTLY to all files...');
...
function applyTemplateStructure(filePath) {
  try {
    ...
    if (content.includes('schema: |') && content.includes('<script type="application/ld+json">')) {
      ...
      // Fix 2: Apply exact template indentation structure
      // The template shows:
      // - Opening { brace: 4 spaces
      // - Top-level properties: 4 spaces  
      // - Nested object properties: 8 spaces
      // - Deeply nested properties: 12 spaces
      
      // Fix script tag indentation (should be 4 spaces)
      fixedContent = fixedContent.replace(/^    <script type="application\/ld\+json">$/gm, '    <script type="application/ld+json">');
      
      // Fix opening brace indentation (should be 4 spaces)
      fixedContent = fixedContent.replace(/^  \{$/gm, '    {');
      
      // Fix top-level properties (should be 4 spaces)
      fixedContent = fixedContent.replace(/^  (\s*"[^"]*":\s*[^,\n]*,?)$/gm, '        $1');
      fixedContent = fixedContent.replace(/^  (\s*"[^"]*":\s*\{)$/gm, '        $1');
      fixedContent = fixedContent.replace(/^  (\s*\},?)$/gm, '        $1');
      fixedContent = fixedContent.replace(/^  (\s*\})$/gm, '        $1');
      
      // Fix nested object properties (should be 8 spaces)
      fixedContent = fixedContent.replace(/^        (\s*"[^"]*":\s*[^,\n]*,?)$/gm, '            $1');
      fixedContent = fixedContent.replace(/^        (\s*"[^"]*":\s*\{)$/gm, '            $1');
      fixedContent = fixedContent.replace(/^        (\s*\},?)$/gm, '            $1');
      fixedContent = fixedContent.replace(/^        (\s*\})$/gm, '            $1');
      
      // Fix deeply nested properties (should be 12 spaces)
      fixedContent = fixedContent.replace(/^            (\s*"[^"]*":\s*[^,\n]*,?)$/gm, '                $1');
      fixedContent = fixedContent.replace(/^            (\s*"[^"]*":\s*\{)$/gm, '                $1');
      fixedContent = fixedContent.replace(/^            (\s*\},?)$/gm, '                $1');
      fixedContent = fixedContent.replace(/^            (\s*\})$/gm, '                $1');
...
function validateTemplateStructure(filePath) {
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (inJsonBlock && line.trim()) {
        // Check opening brace (should be 4 spaces)
        if (line.match(/^  \{$/)) {
          issues.push('opening brace indentation');
          break;
        }
        
        // Check top-level properties (should be 4 spaces)
        if (line.match(/^  [^ ]/) && !line.match(/^    <script/)) {
          issues.push('top-level property indentation');
          break;
        }
        
        // Check nested properties (should be 8+ spaces)
        if (line.match(/^        [^ ]/) && line.includes('"')) {
          issues.push('nested property indentation');
          break;
        }
      }
    }
...
function main() {
  try {
    ...
    console.log(`🎯 Target: Exact template structure (4-space opening brace, 4-space properties, 8-space nested)\n`);
    
    // Phase 1: Fix all files
    console.log('🔧 Phase 1: Applying template structure...');
    files.forEach((filePath, index) => {
      ...
      const result = applyTemplateStructure(filePath);
      if (result.fixed) {
        fixedCount++;
    ...
    console.log(`\n🔍 Phase 2: Validating template structure...`);
    ...
    files.forEach((filePath, index) => {
      ...
      const validation = validateTemplateStructure(filePath);
    ...
    if (validationErrors === 0) {
      ...
      console.log(`\n⚠️  ${validationErrors} files still have issues. Running again...`);
      return false; // Indicate we need to run again
    }
